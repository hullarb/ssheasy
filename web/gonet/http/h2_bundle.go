...
// use by the webssh/web/net/http package (from Go 1.6 and later).
...
// during the back-and-forth between webssh/web/net/http and x/net/http2 (when the
// webssh/web/net/http.Transport is upgraded to also speak http2), as well as support
// the case where x/net/http2 is being used directly.
func (p *http2clientConnPool) shouldTraceGetConn(st http2clientConnIdleState) bool {
	// If our Transport wasn't made via ConfigureTransport, always
	// trace the GetConn hook if provided, because that means the
	// http2 package is being used directly and it's the one
	// dialing, as opposed to webssh/web/net/http.
	if _, ok := p.t.ConnPool.(http2noDialClientConnPool); !ok {
		return true
	}
	// Otherwise, only use the GetConn hook if this connection has
	// been used previously for other requests. For fresh
	// connections, the webssh/web/net/http package does the dialing.
	return !st.freshConn
}
...
// ConfigureServer adds HTTP/2 support to a webssh/web/net/http Server.
//
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	protoHandler := func(hs *Server, c *tls.Conn, h Handler) {
		...
		// the webssh/web/net/http package passes down its per-connection
		// base context via an exported but unadvertised
		// method on the Handler. This is for internal
		// webssh/web/net/http<=>http2 use only.
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	// The webssh/web/net/http package sets the write deadline from the
...
type http2serverConn struct {
	...
	tlsState         *tls.ConnectionState        // shared by all handlers, like webssh/web/net/http
...
func (sc *http2serverConn) maxHeaderListSize() uint32 {
	...
	// So, take the webssh/web/net/http.Server value and pad it up a bit, assuming 10 headers.
...
// setConnState calls the webssh/web/net/http ConnState hook for this connection, if configured.
// Note that the webssh/web/net/http package does StateNew and StateClosed for us.
...
func http2isClosedConnError(err error) bool {
	...
	// have a way to bundle this into std's webssh/web/net/http somehow.
...
func (sc *http2serverConn) wroteFrame(res http2frameWriteResult) {
	...
	if http2writeEndsStream(wr.write) {
		...
		case http2stateOpen:
			...
			// the webssh/web/net/http package provides no mechanism
...
func (sc *http2serverConn) processHeaders(f *http2MetaHeadersFrame) error {
	...
	// The webssh/web/net/http package sets the read deadline from the
...
func (rws *http2responseWriterState) writeChunk(p []byte) (n int, err error) {
	...
	if !rws.sentHeader {
		...
		if _, ok := rws.snapHeader["Date"]; !ok {
			// TODO(bradfitz): be faster here, like webssh/web/net/http? measure.
...
// checkWriteHeaderCode is a copy of webssh/web/net/http's checkWriteHeaderCode.
...
// ConfigureTransport configures a webssh/web/net/http HTTP/1 Transport to use HTTP/2.
...
// needs to be detected by webssh/web/net/http regardless of whether it's its
// bundled version (in h2_bundle.go with a rewritten type name) or
// from a user's x/net/http2. As such, as it has a unique method name
// (IsHTTP2NoCachedConnError) that webssh/web/net/http sniffs for via func
// isNoCachedConnError.
...
// or its equivalent renamed type in webssh/web/net/http2's h2_bundle.go. Both types
...
// errRequestCanceled is a copy of webssh/web/net/http's errRequestCanceled because it's not
...
func (cc *http2ClientConn) roundTrip(req *Request) (res *Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	// TODO(bradfitz): this is a copy of the logic in webssh/web/net/http. Unify somewhere?
...
// a "content-length" request header. This logic is basically a copy of the webssh/web/net/http
...
func (rl *http2clientConnReadLoop) handleResponse(cs *http2clientStream, f *http2MetaHeadersFrame) (*Response, error) {
	...
	if statusCode >= 100 && statusCode <= 199 {
		cs.num1xx++
		const max1xxResponses = 5 // arbitrary bound on number of informational responses, same as webssh/web/net/http
...
func (b http2transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("webssh/web/net/http: server replied with more than declared Content-Length; truncated")
...
// (The field is exported so it can be accessed via reflect from webssh/web/net/http; tested
